...
console.log("game.tetramino is: " + game.tetramino + ".JSON.stringify(game.to_shape()) is: " + JSON.stringify(game.to_shape()))

const Keybind = { 'keydown': {}, 'keyup': {} }

var Config = {
    'das': 100, 'arr': 0, 'delay': 0, 'pressing_left': false, 'pressing_right': false, 'pressing_down': false, 'pressing': {},
    'skim_ind': true, 'mdhole_ind': false, 'unqiue_ind': true, 'auto_next_ind': true,
    'mode': 'Path Mode', 'no_of_unreserved_piece': 7, 'no_of_piece': 7,
...
function get_next_path_minimal_queue(queues) {
    ...
    if(distributedQueues) {
        ...
        //there has been a puzzle in that slot.
        for (let i = 0; i< queues.length; i++) {
            console.log('in get_next_path_minimal_queue')
            for(let j = 0; j < queues[i].length; j++) {
                if (queues[i][j] != 0) {
                    i+=1;
                } else if (queues[i][j] == 0){
                    console.log('queue is ' + pathMinimalQueues[i][j].split('') + 'weight table is ' + pathMinimalQueuesWeightTable[i])
                    console.log(pathMinimalQueuesWeightTable)
                    pathMinimalQueuesWeightTable[i][j] += 1;
                    return pathMinimalQueues[i][j].split('');
                }
            }
        }
    } else {
